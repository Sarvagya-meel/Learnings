...
def serialize_transcription_segments(speech_segments):
    """
    Extracts call transcription segments to a list of dicts.
    """
    transcription_data = []
    txt = ""
    for seg in speech_segments:
        txt = txt+f"\n{seg.segmentSpeaker}:{seg.segmentText}"
        transcription_data.append({"speaker": seg.segmentSpeaker, "text": seg.segmentText})
        # transcription_data.append({            "start_time": seg.segmentStartTime,
        #     "end_time": seg.segmentEndTime,
        #     "speaker": seg.segmentSpeaker,
        #     "text": seg.segmentText,
        #     "confidence": [w["confidence"] for w in seg.segmentConfidence],
        #     # Optionally, include more fields if needed
        # })

    return txt, transcription_data
...
def generate_document():
    ...
    return serialize_transcription_segments(speech_segments)

if __name__ == "__main__":
    transcription_text,transcription_json =  generate_document()
    print(transcription_text)
    from llm_generation import llm_generate
    speakers = llm_generate(transcript=transcription_json)
    # print(speakers)
    # Enrich each entry with the 'name'
    for entry in transcription_json:
        speaker = entry['speaker']
        entry['name'] = speakers.get(speaker, 'Unknown')
        print(f"{entry['name']}:{entry['text']}")
    # print(transcription_json)
    # Now, you can print, save, or pass `call_transcription` as needed
    import json
    from datetime import datetime
    time = datetime.now()
    with open(f"call_transcription.json_{time}", "w", encoding="utf-8") as f:
